...
export function SourcePassages({ passages }: { passages: Passage[] }) {
  if (!passages.length) {
    return (
      ...
      {passages.map((p, i) => (
        ...
        >
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {/* is_selected==1 in the dataset — the labelled gold passage.
                Shown because it is genuine ground truth, not a guess. */}
            {p.is_gold && (
              <span className="rounded bg-[var(--green)] px-2 py-0.5 text-xs font-medium text-yellow">
                ★ gold passage
              </span>
            )}
